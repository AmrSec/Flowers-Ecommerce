...
function setupFilters() {
    document.querySelectorAll('#filter-list li').forEach(item => {
        item.onclick = function() {
            ...
            document.querySelectorAll('#filter-list li').forEach(li => li.className = "text-gray-400 hover:text-primary transition-colors py-1 cursor-pointer");
            this.className = "text-primary border-b-2 border-primary py-1 cursor-pointer";
...
function displayProducts(productList) {
    ...
    productList.forEach(product => {
        grid.innerHTML += `
            <div class="product bg-white dark:bg-[#1a1a1a] rounded-xl overflow-hidden shadow-sm group">
                ...
                <div class="p-4">
                    <h4 class="font-bold dark:text-white truncate">${product.title}</h4>
...
window.addToCart = function(id) {
    ...
    if (window.showToast) {
        ...
        window.showNotification(`${product.title} added to cart!`);
    }
};